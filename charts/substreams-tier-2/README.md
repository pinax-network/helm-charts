...
![Version: 1.0.8](https://img.shields.io/badge/Version-1.0.8-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: v1.6.4](https://img.shields.io/badge/AppVersion-v1.6.4-informational?style=flat-square)
...
| image | object | `{"pullPolicy":"IfNotPresent","repository":"ghcr.io/streamingfast/firehose-core","tag":"v1.6.4"}` | Image configuration for substreams tier 2 |
| image.pullPolicy | string | `"IfNotPresent"` | The pull policy for the Docker image. Valid options are "Always", "IfNotPresent", and "Never". |
| image.repository | string | `"ghcr.io/streamingfast/firehose-core"` | The repository where the Docker image is located. |
| image.tag | string | `"v1.6.4"` | The tag of the Docker image to use. If not set, the default tag will be used. |
...
| substreams.aliases | object | `{}` | Define custom aliases for preconfigured commands in your environment. This allows you to create shorthand commands for frequently used operations, enhancing efficiency and reducing the need for repetitive typing. |
| substreams.autoscaling.enabled | bool | `false` | Specifies whether the Horizontal Pod Autoscaler is enabled |
...
| substreams.serviceMonitor.enabled | bool | `false` | Enable or disable the service monitor |
